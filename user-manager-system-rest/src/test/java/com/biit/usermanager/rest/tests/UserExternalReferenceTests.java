...
@Test(groups = "userExternalReferenceTests")
@DirtiesContext(classMode = DirtiesContext.ClassMode.AFTER_CLASS)
public class UserExternalReferenceTests extends AbstractTestNGSpringContextTests {
    ...
    public void getByExternalReferenceUser() throws Exception {
        //Create the admin user
        userController.createUser(USER2_NAME, USER2_UNIQUE_ID, USER2_FIRST_NAME, USER2_LAST_NAME, USER2_PASSWORD, USER2_EXTERNAL_REFERENCE, null);

        final MvcResult userResult = this.mockMvc
                .perform(get("/users/references/" + USER2_EXTERNAL_REFERENCE)
    ...
    @Test(dependsOnMethods = {"login", "getByExternalReferenceUser"})
    public void getMultiplesByExternalReferenceUser() throws Exception {
        //Create the admin user
        userController.createUser(USER3_NAME, USER3_UNIQUE_ID, USER3_FIRST_NAME, USER3_LAST_NAME, USER3_PASSWORD, USER3_EXTERNAL_REFERENCE, null);

        final MvcResult userResult = this.mockMvc
                .perform(get("/users/references").param("references", USER2_EXTERNAL_REFERENCE)
                        .header("Authorization", "Bearer " + jwtToken)
                        .with(csrf()))
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andReturn();
