...
import com.biit.usermanager.persistence.repositories.ServiceRoleRepository;
...
@Service
public class BackendServiceRoleProvider extends CreatedElementProvider<BackendServiceRole, BackendServiceRoleId, ServiceRoleRepository> {

    public BackendServiceRoleProvider(ServiceRoleRepository repository) {
        super(repository);
    }

    public List<BackendServiceRole> findByService(BackendService backendService) {
        return getRepository().findByIdService(backendService);
    }
    ...
    public Optional<BackendServiceRole> findByServiceAndName(BackendService backendService, String name) {
        return getRepository().findByIdServiceAndIdName(backendService, name);
    }
